...
% for t = 1:T
%     p(t) = ilogit(theta(27)+theta(28)*effort(t)+theta(29)*temp(t)+theta(30)*moon(t));
% end

% p(.), p(sex), p(origin), p(sex+origin)
% p(1) = 1;
for t = 1:T
    p(t) = ilogit(theta(21+t));%+theta(23)*effort(t));%+theta(24)*temp(t));%+theta(25)*moon(t));
end
...
for t = 1:T-1
    phi(t)=(ilogit(theta(42)))^time_int(t);
end
phi(T)=0;
