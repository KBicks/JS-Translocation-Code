function [NM,NF,betaM,betaF,phiM,phiF,pM,pF] = paramcalcJS(theta,MWi,FWi)

% specify time intervals and time-varying covariates
...
% for t = 1:T
%     pMT(t) = ilogit(theta(23)+theta(24)*effort(t)+theta(25)*temp(t)+theta(26)*moon(t));
% end
% 
% for t = 1:T
%     pFT(t) = ilogit(theta(23)+theta(24)*effort(t)+theta(25)*temp(t)+theta(26)*moon(t));
% end

% p(1) = 1;
for t = 1:T
    pM(t) = ilogit(theta(21+t));%+theta(23)*effort(t));%+theta(24)*temp(t));%+theta(25)*moon(t));
end

pF = pM;
% 
% for t = 1:T
%     pF(t) = ilogit(theta(27)+theta(28)*effort(t)+theta(29)*temp(t)+theta(30)*moon(t));
% end

% % full time-dependence
% for t = 1:T
%     pF(t) = ilogit(theta(42)+theta(43)*moon(t)+theta(44)*temp(t));
% end
...
for t = 1:T-1
    phiM(t)=(ilogit(theta(42)))^time_int(t);
end



for t = 1:T-1
    phiF(t)=(ilogit(theta(42)))^time_int(t);
end
